                            ...
                            if (err || !result.settings || !result.currencies || result.transaction.nModified == 0 || result.task.nModified == 0) {
                                ...
                                db.GetDocument('emailtemplate', { name: { $in: ['PaymentDetailstoAdmin', 'PaymentDetailstoTasker', 'PaymentDetailstoUser'] }, 'status': { $ne: 0 } }, {}, {}, function (err, template) {
                                    if (err) {
                						...
                						html = html.replace(/{{totalamount}}/g, currencies.symbol + ' ' + ((task.invoice.amount.total + (task.invoice.amount.extra_amount || 0) + task.invoice.amount.service_tax) - (task.invoice.amount.coupon || 0)));
										...
										html2 = html2.replace(/{{totalamount}}/g, currencies.symbol + ' ' + ((task.invoice.amount.total + (task.invoice.amount.extra_amount || 0) + task.invoice.amount.service_tax) - (task.invoice.amount.coupon || 0)));
										...
										html3 = html3.replace(/{{totalamount}}/g, currencies.symbol + ' ' + ((task.invoice.amount.total + (task.invoice.amount.extra_amount || 0) + task.invoice.amount.service_tax) - (task.invoice.amount.coupon || 0)));
                            ...
                            if (err || !value.currencies || value.updateresult.nModified == 0) {
                                ...
                                db.GetOneDocument('task', { _id: data.task }, {}, options, function (err, task) {
                                    if (err) {
                                        ...
                                        var booking_date = timezone.tz(task.history.job_completed_time, settingdata.settings.time_zone).format(settingdata.settings.date_format);
                                        var booking_time = timezone.tz(task.history.job_completed_time, settingdata.settings.time_zone).format(settingdata.settings.time_format);
                                        //datetime = moment(task.history.job_completed_time).format('DD/MM/YYYY - HH:mm');
                                        console.log("Comp Date , Time",booking_date,booking_time);
                                        datetime = booking_date+' - '+booking_time;
                                        //BookingDate = moment(task.history.booking_date).format('DD/MM/YYYY');
                                        BookingDate = timezone.tz(task.history.booking_date, settingdata.settings.time_zone).format(settingdata.settings.date_format);
                                                ...
                                                pdf.create(html, options).toFile('./uploads/invoice/' + pdfname + '.pdf', function (err, document) {
                                                    ...
                                                    if (err) {
                                                        ...
                                                        var mailOptions = {
                                                            ...
                                                            attachments: [{
                                                                filename: 'Militaryhandyman Admin Invoice.pdf',
                                                                path: './uploads/invoice/' + pdfname + '.pdf',
                                                                contentType: 'application/pdf'
                                                            }],
                                                        };
                                                ...
                                                pdf.create(html2, options).toFile('./uploads/invoice/' + pdfname1 + '.pdf', function (err, document) {
                                                    ...
                                                    if (err) {
                                                        ...
                                                        var mailOptions1 = {
                                                            ...
                                                            html: '<b>Please Download the attachment to see Your Invoice</b>',
                                                            attachments: [{
                                                                filename: 'Militaryhandyman ' + CONFIG.USER + ' Invoice.pdf',
                                                                path: './uploads/invoice/' + pdfname1 + '.pdf',
                                                                contentType: 'application/pdf'
                                                            }],
                                                        };
                                                ...
                                                pdf.create(html3, options).toFile('./uploads/invoice/' + pdfname2 + '.pdf', function (err, document) {
                                                    ...
                                                    if (err) {
                                                        ...
                                                        var mailOptions2 = {
                                                            ...
                                                            html: '<b>Please Download the attachment to see Your Invoice</b>',
                                                            attachments: [{
                                                                filename: "MilitaryhandymanServiceProInvoice.pdf",
                                                                path: './uploads/invoice/' + pdfname2 + '.pdf',
                                                                contentType: 'application/pdf'
                                                            }],
                                                        };
