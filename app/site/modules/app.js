...
angular.module('handyforall.site', ['Authentication',
    ...
    .controller('rootCtrl', function (config, $window, $scope, $rootScope, $state, AuthenticationService, MainService, socket, notify, $translate, toastr, $cookieStore, ngMeta, $sce) {
    ...
    }).controller('MainCtrl', function ($scope, $location, $rootScope, $http, toastr, MainserviceResolve, MainService, $state, $translate, $cookieStore ,$sce) {

        var mac = this;
        mac.myInterval = 9000;
        mac.data = MainserviceResolve;
        mac.postheader = mac.data.response[1].PostHeader;
        var set = mac.data.response[2].Settings;
        mac.video = set.settings.site_video_url || "";
        ...
        mac.vidurl = "https://www.youtube.com/embed/"+mac.video;

        mac.videourl = function (video){
    ...
    .directive('errFile', function () {
        return {
            link: function (scope, element, attrs) {
                element.bind('error', function () {
                    if (attrs.src != attrs.errSrc) {
                        attrs.errSrc = "uploads/images/task/pdffile.png";
                        attrs.$set('src', attrs.errSrc);
                    }
                });
            }
