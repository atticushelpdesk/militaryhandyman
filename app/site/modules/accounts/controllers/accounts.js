...
accountsCtrl.$inject = ['$scope', '$rootScope' , 'MainService', 'accountService', 'accountServiceResolve', '$filter', '$uibModal', '$location', 'toastr', '$timeout', 'Slug', '$state', '$window', '$anchorScroll', 'AuthenticationService', 'sweet', '$stateParams', '$translate', 'NgMap', 'Lightbox'];
function accountsCtrl($scope, $rootScope , MainService, accountService, accountServiceResolve, $filter, $uibModal, $location, toastr, $timeout, Slug, $state, $window, $anchorScroll, AuthenticationService, sweet, $stateParams, $translate, NgMap, Lightbox) {
	...
	if (accountServiceResolve[0]) {
		...
		if(acc.user.accurateBackground){
			acc.accurate = acc.user.accurateBackground.accurateCheck || 3;
		}
	}
	...
	acc.openLightboxModal = function (index, image) {
	...
	acc.accurateModal = function () {
		var name = acc.user.name, email = acc.user.email;
		console.log("AccurateModal",name,email);
		if(name && name.first_name && name.last_name){
			if(email){
				accountService.accurateBackground(name.first_name,name.last_name,email).then(function(res){
					console.log("Response",res);
					if(res && res.status == 1){
						acc.accurate = 2;
						toastr.success("Data Submitted For Background Check Successfully");
					}else{
			...
			toastr.error("Please Enter First Name & Last Name");
		}


		/* var modalInstance = $uibModal.open({
			animation: true,
			templateUrl: 'app/site/modules/accounts/views/accurate.modal.tab.html',
			controller: 'AccurateModalInstanceCtrl',
			controllerAs: 'AMI',
			resolve: { }
		});

		modalInstance.result.then(function (accurate) {
			console.log("AccurateData",accurate);
			if(accurate.firstname,accurate.lastname,accurate.email){
				accountService.accurateBackground(accurate).then(function(res){
					console.log("Response",res);
					if(res && res.status == 1){
						toastr.success("Data Submitted For Background Check Successfully")
					}else{
	...
	acc.accurateCheck = function () {
		console.log("Accurate Check",acc.user.email);
		accountService.accurateManual(acc.user.email).then(function(res){
			console.log("Response",res);
			if(res && res.status == 1){
				acc.accurate = 1;
				toastr.success("Data Verified Successfully")
			}else{
				toastr.error(res.response)
			}
		})
	...
	acc.getUserWalletTransaction = function getUserWalletTransaction() {
		...
		accountService.getUserWalletTransaction(acc.user._id, acc.walletListCurrentPage, acc.walletListitemsPerPage).then(function (response) {
			if (response) {
				...
				var transac = [];
				if(acc.totalitem.length > 0){
					acc.totalitem.forEach(function(element){
						if(element.trans_amount > 0){
							transac.push(element);
						}
					})
					acc.totalitem = transac;
				}

				console.log("Transac",transac)
				console.log("acc.totalitem",acc.totalitem)
				acc.SubCategoryList = transac;
	...
	acc.TaskInviteViewModal = function (index) {
		var modalInstance = $uibModal.open({
			...
			resolve: {
				TaskInvite: function () {
					console.log("acc.taskInvitation[index]",acc.taskInvitation[index]);
					return acc.taskInvitation[index];
				},
				DefaultCurrency: function () {
					return $scope.DefaultCurrency;
	...
	acc.taskerconfirmtask = function (id, taskerid, status) {
		console.log(id,taskerid,status);
		var modalInstance = $uibModal.open({
			animation: true,
...
/* angular.module('handyforall.accounts').controller('AccurateModalInstanceCtrl', function ($uibModalInstance, toastr, $translate) {
	var ami = this;
	ami.pattern = "^[_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,})$";
	ami.accurate = {};
	ami.accurate.firstname = "";
	ami.accurate.lastname = "";
	ami.accurate.email = "";
	ami.ok = function (accurate,valid) {
		console.log("accurate,valid",accurate,valid)
		if (valid) {
			$uibModalInstance.close(accurate);
		} else {
			$translate('FORM IS INVALID').then(function (headline) { toastr.error(headline); }, function (translationId) { toastr.error(headline); });
		}
	};
