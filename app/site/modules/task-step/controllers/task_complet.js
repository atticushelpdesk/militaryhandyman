...
function taskFilterCtrl($scope, $rootScope, $location, $stateParams, SearchResolve, TaskService, TaskserviceResolve, toastr, $state, $filter, AuthenticationService, $modal, MainService, TaskServiceNewResolve, $translate, ngMeta, TaskerCountResolve, NgMap, $scope, $q, $log, $uibModal) {
	...
	tfc.grandPage = TaskerCountResolve.count;
	tfc.grandCurrent = "";
	tfc.grandItem = "";
	tfc.grandTotal = "";
	tfc.checkboxes = [];
	...
	*/
	...
	tfc.getTaskerDetails = function () {
		console.log("cvbcbvc@@@");
		//console.log("Checkboxes", tfc.checkboxes);
		console.log("tfc.UIkmslide --- tfc.UIslide", tfc.UIslide,tfc.UIkmslide);
		if (tfc.UIslide) {
		...
		if ($rootScope.currentState.name != 'search') {
			...
			if (tfc.viewType == 'list') {
				console.log("getTaskerByGeoFilter", tfc.filter, tfc.currentPage, tfc.itemsPerPage);
				TaskService.getTaskerByGeoFilter(tfc.filter, tfc.currentPage, tfc.itemsPerPage).then(function (response) {
					...
					tfc.grandCurrent = tfc.currentPage;
					tfc.grandItem = tfc.itemsPerPage;
					tfc.grandTotal = tfc.totalItem;
					//console.log("TOTAL COUNT", tfc.grandItem, tfc.grandCurrent, tfc.grandTotal, tfc.grandPage);
					if (tfc.checkboxes.length <= 0) {
						for (var t = 1; t <= tfc.grandPage; t++) {
							var subcheck = [];
							for (var k = 0; k < tfc.grandItem; k++) {
								subcheck.push({
									details: false,
									selected: false
								})
							}
							tfc.checkboxes.push(subcheck);
						}
					}
					console.log("tfc.checkboxes", tfc.checkboxes)
					if (angular.isDefined(response.result)) {
						tfc.TaskerDetails = response.result;
						console.log("tfc.TaskerDetails", tfc.TaskerDetails)
						/* if (tfc.TaskerDetails.length > 0) {
							for (let k in tfc.TaskerDetails) {
								tfc.checkboxes.push({
									details: false,
									selected: false
								})
	...
	tfc.onCheckChange = function onCheckChange(check, index, content) {
		//console.log(check, index, content);
		if (check) {
			tfc.checkboxes[tfc.grandCurrent - 1][index].details = content;
		} else {
			tfc.checkboxes[tfc.grandCurrent - 1][index].details = false;
		}
		//console.log("Result", tfc.checkboxes);
	}

	tfc.onAllCheck = function onAllCheck(bool, content) {
		//console.log("Bool", bool, "Content", content);
		if (bool) {
			tfc.checkboxes[tfc.grandCurrent - 1] = [];
			tfc.TaskerDetails.forEach(function(element){
				var details = {
					tasker: element,
					btime: content.btime,
					bdate: content.bdate,
				...
				tfc.checkboxes[tfc.grandCurrent - 1].push({
					details:details,
					selected: true
				});
			})
	...
	tfc.onRequestAll = function onRequestAll(multiCheckBox) {
		var multitasker = multiCheckBox.flat();
		//var finalRequest = [];
		var submult = multitasker.filter(function(task){return task.selected == true})
		//console.log("SUBMULT",submult);
		if (submult.length > 0) {
			...
			modalInstance.result.then(function () {
				submult.forEach(function(content){
					if (content.selected == true) {
						var message = content.details;
						var date = {}, allTasker = {};
						date.currectdate = tfc.filter.date;
						date.time = tfc.filter.time;
						allTasker.tasker = message.tasker._id;
						allTasker.invoice = {
							'amount': {
						...
						allTasker.history = {};
						allTasker.history.job_booking_time = new Date();
						allTasker.history.est_reach_date = '';
						allTasker.status = 1;
						allTasker.billing_address = tfc.taskinfo.billing_address;
						//allTasker.booking_id = tfc.taskinfo.booking_id;
						allTasker.category = tfc.taskinfo.category;
						allTasker.createdAt = tfc.taskinfo.createdAt;
						allTasker.location = tfc.taskinfo.location;
						allTasker.payee_status = tfc.taskinfo.payee_status;
						allTasker.reject_job = tfc.taskinfo.reject_job;
						allTasker.task_address = tfc.taskinfo.task_address;
						allTasker.task_date = tfc.taskinfo.task_date;
						allTasker.task_day = tfc.taskinfo.task_day;
						allTasker.task_description = tfc.taskinfo.task_description;
						allTasker.transactions = tfc.taskinfo.transactions;
						allTasker.updatedAt = tfc.taskinfo.updatedAt;
						allTasker.user = tfc.taskinfo.user;
						allTasker.photo = tfc.taskinfo.photo;
						//allTasker._id = tfc.taskinfo._id;
						...
						allTasker.hourly_rate = tfc.hour_rate || ""
						var taskdatetime = new Date();
						allTasker.task_hour = tfc.filter.hour;
						//finalRequest.push({ data: allTasker, time: date })
						TaskService.requestalltask({ data: allTasker }).then(function (result) {
						}, function (error) {
							toastr.error(error);
						});
					}
				})
				$translate('Request Sent To Selected Service Pros Successfully').then(function (headline) { toastr.success(headline); }, function (translationId) { toastr.success(headline); });
				$state.go('landing', { reload: false });
				//tfc.sendRequestAll(finalRequest);
			});
		} else {
			$translate('Please Select Atleast One Service Pro To Continue').then(function (headline) { toastr.info(headline); }, function (translationId) { toastr.success(headline); });
		}
	}

	/* tfc.sendRequestAll = function sendRequestAll(request) {
		console.log(request);
		if (request.length > 0) {
			/* TaskService.requestalltask(request).then(function (result) {
				console.log("RESULT", result);
				$translate('REQUEST HAS BEEN SENT TO SERVICE PRO SUCCESSFULLY').then(function (headline) { toastr.success(headline); }, function (translationId) { toastr.success(headline); });
				//$state.go('landing', { reload: false });
			}, function (error) {
				toastr.error(error);
			});
			...
			$translate('PLEASE SELECT ATLEAST ONE SERVICE PRO TO PROCEED').then(function (headline) { toastr.info(headline); }, function (translationId) { toastr.error(headline); });
		}
		} 
	*/
	...
	tfc.confirmatask = function confirmatask(message) {
		...
		console.log("Confirm Task", message);
		modalInstance.result.then(function () {
			var date = {};
			date.currectdate = tfc.filter.date;
			date.time = tfc.filter.time;
			...
			tfc.taskinfo.photo = tfc.taskinfo.photo;
			if (tfc.filter.time) {
