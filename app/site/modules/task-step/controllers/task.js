...
taskCtrl.$inject = ['$scope', '$rootScope', '$location', '$stateParams', '$uibModal', 'TaskService', 'TaskserviceResolve', 'toastr', '$state', 'AuthenticationService', 'CurrentUserTaskserviceResolve', 'MainService', '$translate', 'ngMeta','$cookieStore', 'FileUploader'];
function taskCtrl($scope, $rootScope, $location, $stateParams, $uibModal, TaskService, TaskserviceResolve, toastr, $state, AuthenticationService, CurrentUserTaskserviceResolve, MainService, $translate, ngMeta, $cookieStore, FileUploader) {
	var tac = this;
	...
	tac.addFiles = function () {
        document.getElementById('addFiles').click();
    }

    tac.removeFile = function (index) {
		tac.photo.splice(index, 1);
		uploader.queue[index].remove()
        //$cookieStore.put('bookingstep2', atsc.tasker.photo);
        tac.check = 1;
    }

	var uploader = $scope.uploader = new FileUploader({
        url: '/site/account/upload-task-images'
    });
	...
	uploader.filters.push({
		...
		'fn': function (item) {
			return item.size <= 1000000; // 1 MiB to bytes
		}
	});
    ...
    uploader.onWhenAddingFileFailed = function (item /*{File|FileLikeObject}*/, filter, options) { 
		if(filter.name == 'syncFilter'){
			$translate('Unable to upload more than 10 files').then(function (headline) { toastr.error(headline); }, function (translationId) { toastr.error(headline); });
		}
		else if(filter.name == 'enforceMaxFileSize'){
			$translate('Upload file less than 1 MB').then(function (headline) { toastr.error(headline); }, function (translationId) { toastr.error(headline); });
		}
		else{
			$translate('Unable to upload').then(function (headline) { toastr.error(headline); }, function (translationId) { toastr.error(headline); });
		}
	};
    uploader.onAfterAddingFile = function (fileItem) { };
    uploader.onAfterAddingAll = function (addedFileItems) { uploader.uploadAll(); };
    ...
    uploader.onCompleteItem = function (fileItem, response, status, headers) {
        if (response.status == 1) {			
			tac.photo.push(response.response.file);			
...
angular.module('handyforall.task').controller('AddAddress', function ($uibModalInstance, toastr, user, $location, $state, $scope, $translate) {
  ...
  ata.test={};
     ...
     console.log(ata.place);
		...
		ata.editaddressdata.zipcode = '';
		ata.editaddressdata.state = '';
		ata.editaddressdata.city = '';
...
if(user && ata.test == undefined){
	...
	ata.editaddressdata.sat =1;
	 ...
	 if (ata.addressList.location.lat == '' || ata.addressList.location.lng == ''||user.line1=='') {
		 $translate('Please Enter Valid Address Of Task').then(function (headline) { toastr.error(headline); }, function (translationId) { toastr.error(headline); });
	 } else {
	if (isValid == true) {
