...
function customTableCtrl($modal, toastr, $scope, $attrs, MainService) {
    ...
    ctc.filter = function filter(options){
        var modalInstance = $modal.open({
            templateUrl: 'app/admin/modules/common/views/modal/filter.html',
        ...
        modalInstance.result.then(function (selectedItem) {
            ...
            options.getData(ctc.currentPage, ctc.entryLimit, sort, ctc.userTableSearch, selectedItem.state, selectedItem.city,selectedItem.zipcode,selectedItem.country,selectedItem.username,selectedItem.email );
        }, function () {
            //toastr.error('Modal dismissed at: ' + new Date(), 'Error');
        });
    }
...
angular.module('handyforall.admin').controller('filterTestCtrl', filterTestCtrl);
filterTestCtrl.$inject = ['$scope', 'MainService', '$modalInstance', 'toastr'];
function filterTestCtrl($scope, MainService, $modalInstance, toastr) {
    var flt = this;
    $scope.cancel = function () {
        $modalInstance.close();
    };
    $scope.searchtasker = function(){
        $modalInstance.close(flt);
    }
}
