...
function taskersListCtrl(taskersServiceResolve, TaskersService, $scope, toastr, $stateParams) {
    ...
    tlc.table.delete = {
        'date': $scope.date, 'permission': tlc.permission, service: '/taskers/delete', getData: function (currentPage, itemsPerPage, sort, status, search, state, city, zipcode, country, username, email) {
            if (currentPage >= 1) {
                var skip = (parseInt(currentPage) - 1) * itemsPerPage;
                TaskersService.getAllTaskers($scope.statusValue, itemsPerPage, skip, sort, status, search, state, city, zipcode, country, username, email).then(function (respo) {
                    tlc.table.data = respo[0];
