...
function addTaskerCtrl(taskerAddServiceResolve, $filter, $state, TaskersService, $scope, $modal, toastr, CategoryServiceResolve, $timeout, $http, $stateParams, NgMap) {
                ...
                if (isValid) {
                    ...
                    TaskersService.addTaskergeneral(atsc.tasker).then(function (response) {
                        ...
                        if (response == "" || response.msg == "Phone Number Already Exists") {
                            ...
                            toastr.success("Service Pro " + atsc.stateDummyVariable + " Successfully");
    ...
    atsc.accurateCheck = function(){
        console.log("Accurate Check");
        if(atsc.tasker){
            var background = atsc.tasker.accurateBackground;
            if(background){
                if(background.accurateCheck && background.order && background.candidate){
                    console.log("Background",background);
                    atsc.checkStatus = background.accurateCheck;
                    atsc.checkOrder = background.order;
                    atsc.checkCand = background.candidate;
                }
            }
        }
    }

    atsc.accurateCheckNow = function(){
        console.log("Accurate Check Now");
        if(atsc.tasker){
            var email = atsc.tasker.email;
            if(email){
                TaskersService.accurateManual(email).then(function(res){
