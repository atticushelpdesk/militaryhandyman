...
function transactionListCtrl(usersTransactionServiceResolve, UsersService, $scope, MainService, $stateParams) {
    ...
    if (usersTransactionServiceResolve.status == 0) {
        ...
        var trans = usersTransactionServiceResolve.response.trans,transac = [],count = 0;
        var rcount = usersTransactionServiceResolve.response.count || 0;
        for(var i=0;i<trans.length;i++){
            if(trans[i].trans_amount > 0){
                transac.push(trans[i]);
            }else{
                count++
            }
        }
        tlc.table.data = transac;
        tlc.table.count = (rcount - count) ? (rcount - count) : 0;
    }
