module.exports = function (app, io) {
    ...
    router.save = function (req, res) {
        ...
        if (req.body.status == 1) {
            data.accurateBackground = {};
            data.accurateBackground.accurateCheck = 1;
        }
                                                    ...
                                                    if (err) {
                                                        ...
                                                        db.GetDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
                                                            if (err) {
                                                                ...
                                                                if (req.body.status == 1 && tasker.status == 1) {
                                                                    var mailData = {};
    ...
    */
    ...
    router.getrecenttasker = function getrecenttasker(req, res) {
        ...
        if(req.body.country){    
            query['address.country'] =  { $regex: req.body.country + '.*', $options: 'si' };
        }

        if(req.body.city){
            query['address.city'] = { $regex: req.body.city + '.*', $options: 'si' };
        }

        if(req.body.state){
            query['address.state'] = { $regex: req.body.state + '.*', $options: 'si' };
        }

        if(req.body.zipcode){
            query['address.zipcode'] = { $regex: req.body.zipcode + '.*', $options: 'si' };
        }

        if(req.body.username){
            query['username'] = { $regex: req.body.username + '.*', $options: 'si' };
        }

        if(req.body.email){
            query['email'] = { $regex: req.body.email + '.*', $options: 'si' };
        }
            ...
            usersQuery.push({
                "$match": {
                    $or: [
                        { "documentData.username": { $regex: searchs + '.*', $options: 'si' } },
                        { "documentData.email": { $regex: searchs + '.*', $options: 'si' } },
                        { "documentData.address.city": { $regex: searchs + '.*', $options: 'si' } },
                        { "documentData.address.state": { $regex: searchs + '.*', $options: 'si' } },
                        { "documentData.address.zipcode": { $regex: searchs + '.*', $options: 'si' } },
                        { "documentData.address.country": { $regex: searchs + '.*', $options: 'si' } }
    ...
    router.gettaskdetails = function (req, res) {
        var extension = {};
        extension.options = { limit: 10, sort: { 'createdAt': -1 } };
