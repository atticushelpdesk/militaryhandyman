...
module.exports = function (io, res) {
  ...
  controller.fbRegister = function (req, res) {
    ...
    var status = '0';
    var message = '';
    var reg_address = req.body.reg_address;
    var data = {};
      ...
      if (err || !user) { res.send({ "status": "0", "message": res.__("No " + CONFIG.USER + " Found for this Email") }); } else {
        db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
          if (err || !settings) {
            ...
            newdata.address = reg_address;
            newdata.addressList = address;
                        ...
                        for (var i = 0; i < bookings.length; i++) {
                          ...
                          switch (bookings[i].status) {
                            ...
                            case 2:
                              ...
                              break;
                            case 3:
                              bookings[i].job_status = 'Start Billing Clock';
                              break;
                            case 4:
  ...
  controller.register = function (req, res) {
    ...
    //bodyData.replace("'", "\"");
    //var address = JSON.stringify(req.body.reg_address);
    //console.log("address",address,typeof address);
    var reg_address = req.body.reg_address;
    //var reg_address = JSON.parse(address);
    //console.log("reg_address",reg_address,typeof reg_address);
    var data = {};
    data.status = '0';
    var message = '';
          ...
          $group: {
            ...
            category: {
              $push: {
                ...
                image: { $cond: ["$image", { $concat: [settings.settings.site_url, "$image"] }, { $literal: settings.settings.site_url + CONFIG.CATEGORY_DEFAULT_IMAGE }] },
    ...
    db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
      if (err) {
        ...
        /*  if ((data.reach_date == today) && (hour == currenthour)) {
           res.send({
             "status": "0",
             "response": "No Service Pro's are available, Please choose another time"
           });
         } */
        //else {
                                ...
                                db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
                                  if (err) {
                                    ...
                                    for (var i = 0; i < docdata[0].taskers.length; i++) {
                                      ...
                                      if(docdata[0].taskers[i].status == 1){
                                        providertemp.tasker_status = 'Verified';
                                      }
                                      else{
                                        providertemp.tasker_status = 'Unverified';
                                      }
                                      var ratetypes = {};
        ...
        if (req.body.category) {
          var taskercondition = [{
            "$geoNear": {
              ...
              distanceField: "distance",
              includeLocs: "location",
              query: { "status": { $in: [ 1, 2, 3 ] }, "availability": 1, "taskerskills": { $elemMatch: { childid: new mongoose.Types.ObjectId(categoryid), status: 1, hour_rate: { '$gte': minrate, '$lte': maxrate } } }, "current_task": { $exists: false } },
              distanceMultiplier: distanceval,
              spherical: true
            }
          },
          ...
          var taskercondition = [{
            "$geoNear": {
              ...
              distanceField: "distance",
              includeLocs: "location",
              query: { "status": { $in: [ 1, 2, 3 ] }, "availability": 1, "taskerskills": { $elemMatch: { categoryid: new mongoose.Types.ObjectId(maincategoryid), status: 1, hour_rate: { '$gte': minrate, '$lte': maxrate } } }, "current_task": { $exists: false } },
              distanceMultiplier: distanceval,
              spherical: true
            }
          },
          ...
          var taskercondition = [{
            "$geoNear": {
              ...
              distanceField: "distance",
              includeLocs: "location",
              query: { "status": { $in: [ 1, 2, 3 ] }, "availability": 1, "taskerskills": { $elemMatch: { hour_rate: { '$gte': minrate, '$lte': maxrate } } }, "current_task": { $exists: false } },
              distanceMultiplier: distanceval,
              spherical: true
            }
          },
  ...
  controller.multiBooking = function (req, res) {
    console.log("Request Obtained", req.body);
    var errors = [];
    req.checkBody('taskaddress', res.__('Task Address is Required')).notEmpty();
    req.checkBody('taskid', res.__('Task ID is Required')).notEmpty();
    req.checkBody('multitaskers', res.__('Tasker ID is Required')).notEmpty();
    errors = req.validationErrors();
    if (errors) {
      res.send({
        "status": "0",
    ...
    //req.sanitizeBody('taskaddress').trim();
    //req.sanitizeBody('taskid').trim();
    //req.sanitizeBody('multitaskers').trim();
    var multitaskers = req.body.multitaskers;
    var taskid = req.body.taskid;
    var taskaddress = req.body.taskaddress;
    db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settingdata) {
      if (err) {
        ...
        db.GetOneDocument('task', { '_id': taskid }, {}, options, function (err, taskData) {
          //console.log("TASK DATA", taskData);
          var category = taskData.category, user = taskData.user;
          if (err) {
            res.send({ "status": "0", "errors": err });
          } else {
            if (multitaskers.length <= 0) {
              res.send({ "status": "0", "errors": "Empty Request" });
            } else {
              async.each(multitaskers, function (tasks, callback) {
                //console.log("TASKER ID", tasks);
                var task = {};
                task.booking_id = settingdata.settings.bookingIdPrefix + '-' + Math.floor(100000 + Math.random() * 900000);;
                db.GetOneDocument('tasker', { '_id': tasks }, {}, {}, function (err, tasker) {
                  if (err) {
                    callback(err);
                  } else {
                    var skills = tasker.taskerskills;
                    task.tasker = tasker._id;
                    task.invoice = {
                      'amount': {
                        "minimum_cost": category.commision,
                        "task_cost": category.commision,
                        "total": category.commision,
                        "grand_total": category.commision
                      }
                    };
                    task.booking_information = {
                      'service_type': category.name,
                      'work_type': category.name,
                      'work_id': category._id,
                      'instruction': taskData.task_description,
                      'booking_date': new Date(),
                      'reach_date': '',
                      'est_reach_date': '',
                      //'location': taskData.billing_address.line1 + "," + taskData.billing_address.line2 + "," + taskData.billing_address.city + "," + taskData.billing_address.state + "," + taskData.billing_address.country + "," + taskData.billing_address.zipcode
                      'location': user.address.line1 + "," + user.address.line2 + "," + user.address.city + "," + user.address.state + "," + user.address.country + "," + user.address.zipcode
                    }
                    task.billing_address = user.address;
                    //task.billing_address = taskData.billing_address;
                    task.history = {};
                    task.history.job_booking_time = new Date();
                    task.history.est_reach_date = '';
                    task.status = 1;
                    task.category = category._id;
                    task.payee_status = taskData.payee_status;
                    task.reject_job = taskData.reject_job;
                    task.task_address = taskaddress;
                    task.location = taskData.location;
                    task.task_date = taskData.task_date;
                    task.task_day = taskData.task_day;
                    task.task_day = taskData.task_hour;
                    task.task_description = taskData.task_description;
                    task.transactions = taskData.transactions;
                    task.user = user._id;
					task.photo = req.body.photo || [];
                    task.hourly_rate = "";
                    for (var i = 0; i < skills.length; i++) {
                      if (skills[i].childid.toString() == category._id.toString()) {
                        console.log("----Hourly Rate----", skills[i].hour_rate);
                        task.hourly_rate = skills[i].hour_rate
                      }
                    }
                    //console.log("TASK Hourly Rate", task.hourly_rate);
                    //console.log("Insert Task", task);
                    var message = CONFIG.NOTIFICATION.REQUEST_FOR_A_JOB;
                    var notifications = { 'job_id': task.booking_id, 'user_id': task.user };
                    db.InsertDocument('task', task, function (err, docdata) {
                      if (err) {
                        callback(err);
                      } else {
                        var job_date = timezone.tz(task.booking_information.booking_date, settingdata.settings.time_zone).format(settingdata.settings.date_format);
                        var job_time = timezone.tz(task.booking_information.booking_date, settingdata.settings.time_zone).format(settingdata.settings.time_format);
                        var mailcredentials = {};
                        mailcredentials.taskname = category.name + " (" + task.booking_information.work_type + ")";
                        mailcredentials.username = user.username;
                        mailcredentials.taskername = tasker.username;
                        mailcredentials.taskeremail = tasker.email;
                        mailcredentials.useremail = user.email;
                        mailcredentials.bookingid = task.booking_id;
                        mailcredentials.taskdate = job_date;
                        mailcredentials.taskhour = job_time;
                        mailcredentials.taskdescription = task.task_description;
                        var username;
                        var taskername;
                        if (tasker.name) {
                          taskername = tasker.name.first_name + " (" + tasker.username + ")";
                        } else { taskername = tasker.username; }
                        if (user.name) {
                          username = user.name.first_name + " (" + user.username + ")";
                        } else { username = user.username; }
                        //console.log("MailCredentials", mailcredentials);
                        //console.log("Notifications", task.tasker, message, 'job_request', 'ANDROID', notifications, 'PROVIDER');
                        push.sendPushnotification(task.tasker, message, 'job_request', 'ANDROID', notifications, 'PROVIDER', function (err, response, body) { });
                        var mailData = {};
                        mailData.template = 'Taskpendingapproval';
                        mailData.to = mailcredentials.useremail;
                        mailData.html = [];
                ...
                if (err) {
                  ...
                  console.log("Success");
                  res.send({ "status": "1", "response": "Request Sent To Selected Service Pros Successfully" });
                }
              })
            }
          }
  ...
  controller.insertAddress = function (req, res) {
    console.log("INSERT ADDRESS", req.body);
    var errors = [];
    req.checkBody('user_id', res.__(CONFIG.USER + ' ID is Required')).notEmpty();
  ...
  controller.mapinsertAddress = function (req, res) {
    console.log("MAP INSERT ADDRESS", req.body);
    //  city:required city
  ...
  controller.listAddress = function (req, res) {
    console.log("List Address Request", req.body);
    var errors = [];
    req.checkBody('user_id', res.__(CONFIG.USER + ' ID is Required')).notEmpty();
    errors = req.validationErrors();
    if (errors) {
    ...
    try {
      ...
      data.user_id = req.body.user_id;
      db.GetDocument('users', { _id: req.body.user_id }, {}, {}, function (addErr, addRespo) {
        console.log("List Address USERS", addRespo[0]);
                ...
                if (addRespo[0].addressList.length >= 1) {
                  for (var key = 0; key < addRespo[0].addressList.length; key++) {
                    if (key == req.body.address_name) {
                      ...
                      console.log("List Address Result", addressArr);
                      res.send({
                        "status": "1",
                        "response": addressArr
                      });
  ...
  controller.listAddressforandroid = function (req, res) {
    console.log("List Address Android Request", req.body);
    var errors = [];
    req.checkBody('user_id', res.__(CONFIG.USER + ' ID is Required')).notEmpty();
    ...
    var addressArr = [];
    db.GetDocument('users', { _id: req.body.user_id }, {}, {}, function (addErr, addRespo) {
      console.log("List Address Android USERS", addRespo[0].addressList);
        ...
        if (Address_name < Addresslength) {
          for (var key = 0; key < addRespo[0].addressList.length; key++) {
            if (key == req.body.address_name) {
              ...
              console.log("List Address Android RESULT", addressArr);
              res.send({
                "status": "1",
                "response": addressArr
              });
                ...
                if (err || !settings) {
                  ...
                  db.GetAggregation('task', [
                    ...
                    if (err || bookings.length == 0) {
                      ...
                      for (var i = 0; i < bookings.length; i++) {
                        console.log("BOOKING SERVICE ICON", bookings[i]);
                        if (bookings[i].service_icon) {
                          bookings[i].service_icon = settings.settings.site_url + bookings[i].service_icon;
                        }
  ...
  controller.viewJob = function (req, res) {
    ...
    async.waterfall([
      ...
      console.log("taskertaskertaskertaskertaskertaskertaskertasker", tasker);
      ...
      console.log("-=-=-=-=-=-----ooo", task.booking_information)
      ...
      console.log("=-=-=-=bInformation-=-==--=", bInformation.user_latlong)
      var do_cancel = "No";
      switch (task.status.toString()) {
        ...
        case '2':
          ...
          break;
        case '3':
          request.status = 'Start Billing Clock';
          request.btn_group = 3;
          break;
        case '4':
      ...
      var jobsArr = {
        ...
        'photo': task.photo,
        'url': settings.settings.site_url+'uploads/images/task/',
        'do_cancel': do_cancel,
        ...
        async.parallel({
          ...
          if (err || !result || !result.task) {
            ...
            db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
              if (err || !currencies) {
                ...
                jobArr = {
                  ...
                  latitude: result.task.booking_information['user_latlong']['lat'],
                  longitude: result.task.booking_information['user_latlong']['lon'],
                  photo: result.task.photo,
                  url: result.settings.settings.site_url+'uploads/images/task/'
                };
            ...
            billing.push({ response });
                ...
                //   billing.push({ response });
                // }
              //   if (result.task.invoice.miscellaneous  && result.task.invoice.miscellaneous.length !== 0) {
              //     var response = { 'title': 'Miscellaneous amount', 'dt': '1', 'amount': ''};
              //     billing.push({ response });
              // }
              if (result.task.invoice.miscellaneous  && result.task.invoice.miscellaneous.length !== 0) {
                  for (var i = 0; i < result.task.invoice.miscellaneous.length; i++) {
                  var response = { 'title': result.task.invoice.miscellaneous[i].name, 'dt': '1', 'amount': (result.task.invoice.miscellaneous[i].price * currencies.value).toFixed(2) };
                  billing.push({ response });
                  }
              }
          ...
          if (err || !bookings) {
            ...
            db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function (err, currencies) {
              if (err || !currencies) {
                ...
                //   if (bookings.invoice.miscellaneous && bookings.invoice.miscellaneous.length !== 0) {
                //     var billing = { 'title': 'Miscellaneous amount', 'dt': '1', 'amount': ''};
                //     data.response.job.billing.push(billing);
                // }
                if (bookings.invoice.miscellaneous  && bookings.invoice.miscellaneous.length !== 0) {
                    for (var i = 0; i < bookings.invoice.miscellaneous.length; i++) {
                    var billing = { 'title': bookings.invoice.miscellaneous[i].name, 'dt': '1', 'amount': (bookings.invoice.miscellaneous[i].price * currencies.value).toFixed(2) };
                    data.response.job.billing.push(billing);
                    }
                }
                      ...
                      if (err) {
                        ...
                        db.GetAggregation('tasker', getQuery, function (err, docdata) {
                          if (err) {
                            ...
                            db.GetDocument('review', { 'tasker': provider_id, 'type': 'user' }, {}, {}, function (err, totalreview) {
                              if (err) {
                                res.send({
                                  "status": "0",
                                  "response": res.__("You are not valid totalreview")
                                });
                              } else {
                                var num_of_ratingstt = 0;
                                var totalRatingstt = 0;
                                var avg_ratingtt = 0;
                                for (var i = 0; i < totalreview.length; i++) {
                                  totalRatingstt = totalRatingstt + totalreview[i].rating;
                                  num_of_ratingstt++;
                                }
                                avg_ratingtt = totalRatingstt / num_of_ratingstt;

                                //var avgreview = parseFloat(docdata[0].avg);
                                //var totalreview = docdata[0].documentData[0].task.length;

                                var avgreview = parseFloat(avg_ratingtt);
                                var totalreview = num_of_ratingstt;

                                console.log("avgreview///////////", avg_ratingtt);
                                console.log("totalreview///////////", num_of_ratingstt);
                                console.log("docdata[0].documentData[0] ///////////", docdata[0].count);
                                //console.log("countreview///////////", docdata[0].countreview);
                                console.log("countreview ///////////", docdata[0].induvidualrating);
                                ...
                                db.UpdateDocument('tasker', { _id: provider_id }, { $set: { 'avg_review': avgreview, 'total_review': totalreview } }, { multi: true }, function (userUErr, userURespo) {
                                  if (userUErr) {
                                    ...
                                    var response = {};
                                    response.msg = res.__('Your Rating Has Been Submitted');
                                    response.image = settings.settings.site_url + 'uploads/images/users/' + data.img_name;
                                    res.send({
                                      'status': '1',
                                      'response': response
  ...
  */
          ...
          db.GetOneDocument('users', { 'username': req.body.username, 'email': email }, {}, {}, function (err, user) {
            if (err || !user) {
              ...
              if (user) {
                ...
                newUser.status = 1;
                newUser.address = reg_address;
                newUser.addressList = address;
                    ...
                    { "$limit": request.perPage }
                      ...
                      if (err || bookings.length == 0) {
                        ...
                        for (var i = 0; i < bookings.length; i++) {
                          ...
                          var xxx_amount = parseFloat((bookings[i].invoice.amount.total || 0).toFixed(2));
                          var xxx_extra = parseFloat(bookings[i].invoice.amount.extra_amount || 0).toFixed(2);
                          var xxxservice = parseFloat(bookings[i].invoice.amount.service_tax || 0).toFixed(2);
                          var xxx_discount = parseFloat(bookings[i].invoice.amount.discount || 0).toFixed(2);
                          var xxx_sum = parseFloat(parseFloat(xxx_amount) + parseFloat(xxx_extra) + parseFloat(xxxservice) - parseFloat(xxx_discount));

                          job.total_amount = xxx_sum.toFixed(2);
                    ...
                    { $unwind: "$category" }
                      ...
                      if (err || bookings.length == 0) {
                        ...
                        for (var i = 0; i < bookings.length; i++) {
                          ...
                          var xxx_sum = parseFloat(parseFloat(xxx_amount) + parseFloat(xxx_extra) + parseFloat(xxxservice) - parseFloat(xxx_discount));
                          job.total_amount = xxx_sum.toFixed(2);
                          //job.total_amount = ((bookings[i].invoice.amount.grand_total * currencies.value) - bookings[i].invoice.amount.discount).toFixed(2) || '';
                          if (bookings[i].user) {
                          ...
                          if (bookings[i].invoice.miscellaneous) {
                            job.miscellaneous = bookings[i].invoice.miscellaneous;
                          }else{
                            job.miscellaneous = [];
                          }
  ...
  controller.uploadTaskImages = function uploadTaskImages(req, res) {
    var data = {};
    data.status = '0';
    if (req.files) {
      db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
        if (err || !settings) {
          ...
          for(var i=0; i<req.files.length; i++){
            data.name.push(req.files[i].filename);
          }
          data.url = settings.settings.site_url + 'uploads/images/task/';
          res.send(data);
      ...
      res.send({ "status": 0, "response": res.__('Invalid input') });
    }
  }
