...
module.exports = function (io,i18n) {
    ...
    controller.accurateBackground = function accurateBackground(req, res) {
        ...
        req.checkBody('firstName', res.__('First Name is Required')).notEmpty();
        req.checkBody('lastName', res.__('Last Name is Required')).notEmpty();
        req.checkBody('email', res.__('Email is Required')).notEmpty();
        var errors = req.validationErrors();
        if (errors) {
            ...
            if (err || !taskerac) {
                ...
                acc.candidate(formData, taskerid, function (err, response) {
                    if (err || !response) {
                        ...
                        data.status = 1;
                        data.response = "Data Submitted For Background Check Successfully";
                        res.send(data);
                    }
                });
            }
