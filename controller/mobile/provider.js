module.exports = function(io, i18n) {
    ...
    controller.login = function(req, res) {
        ...
        if (errors) { data.response = errors[0].msg;
    ...
    controller.providerInfo = function(req, res) {
        ...
        req.sanitizeBody('provider_id').trim();
        var job_id = req.body.job_id || false;
        var request = {};
        request.provider_id = req.body.provider_id;
        db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function(err, settings) {
            if (err) {
                    ...
                    if (err) {
                        ...
                        db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function(err, currencies) {
                            if (err || !currencies) {
                                ...
                                var hourly_rate = '',amount = '';
                                var extension = {};
                                extension.populate = 'category';
                                if(job_id){
                                    db.GetOneDocument('task',{'booking_id':job_id},{},extension,function(err,job){
                                        if (err || !job) {
                                            res.send({
                                                "status": 0,
                                                "message": res.__('Error')
                                            ...
                                            //console.log("JOB",job);
                                            hourly_rate = job.hourly_rate, invoice = job.category.commision || false;
                                            if(invoice){
                                                amount = invoice;
                                                console.log("amount,hourly_rate",amount,hourly_rate);
                                            }
                                        }
                                    })
                                }
                                ...
                                if (providers) {
                                    var category = providers.taskerskills.map(function(item) {
                                        if (item.childid) {
                                            ...
                                            catdet.minimum_cost = item.childid.commision;
                                            return catdet;
                                        } else {
                                            return "";
                                        }
                                    ...
                                    for (i = 0; i < category.length; i++) {
                                        ...
                                        var minimumcost = category[i].minimum_cost;


                                        catdet.push({ 'categoryname': categoryname, 'hourlyrate': hourlyrate,'minimum_cost': minimumcost, '_id': id });
                                    }
                                    ...
                                    GoogleAPI.geocode(geocode, function(response) {
                                        ...
                                        if(job_id){
                                            data.response.hourly_rate = hourly_rate;
                                            data.response.minimum_cost = amount;
                                        }
                                ...
                                db.UpdateDocument('tasker', { '_id': request.provider_id }, { 'otp': new_otp }, {}, function(err, users) {
                                    if (err) {
                                        ...
                                        db.GetOneDocument('settings', { 'alias': 'sms' }, {}, {}, function(err, settings) {
                                            if (err || !settings) { data.response = 'Unable to Sent OTP for you';
                                                res.send(data); } else {
        ...
        async.waterfall([
            function(callback) {
                db.GetOneDocument('tasker', { $or: [{ 'email': request.email }, { 'phone.number': request.email }] }, {}, {}, function(err, user) {
                    if (err || !user) { data.response = res.__('No ' + CONFIG.TASKER + ' Found for this Phone or Email');
                        res.send(data); } else { callback(err, user); }
                });
            },
            function(user, callback) {
                db.UpdateDocument('tasker', { '_id': user._id }, { 'reset_code': request.reset }, {}, function(err, response) {
                    if (err || response.nModified == 0) { data.response = res.__('Unable to update your reset code');
                        res.send(data); } else { callback(err, user); }
                });
            },
            function(user, callback) {
                        ...
                        db.GetCount('task', query, function(err, count) {
                            if (err || count == 0) {
                                ...
                                db.GetAggregation('task', [
                                    ...
                                    if (err || bookings.length == 0) {
                                        res.send(data);
                                        console.log(data.response.jobs, "4");
                                    } else {
                                        ...
                                        for (var i = 0; i < bookings.length; i++) {
                                            ...
                                            switch (bookings[i].status) {
                                                ...
                                                case 2:
                                                    ...
                                                    break;
                                                case 3:
                                                    request.job_status = 'Start Billing Clock';
                                                    request.btn_group = 3;
                                                    break;
                                                case 4:
        ...
        async.waterfall([
            function(callback) {
                db.GetOneDocument('tasker', { '_id': request.provider_id }, {}, {}, function(err, tasker) {
                    if (err || !tasker) { data.response = res.__('Invalid ' + CONFIG.TASKER + ', Please check your data');
                        res.send(data); } else {
                        callback(err, tasker);
                    }
                });
            ...
            function(tasker, ratings, callback) {
                ...
                db.GetOneDocument('task', { 'booking_id': request.job_id }, {}, extension, function(err, task) {
                    if (err || !task) { data.response = res.__('Jobs Not Available');
                        res.send(data); } else { callback(err, tasker, ratings, task); }
                });
            },
            function(tasker, ratings, task, callback) {
                db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function(err, settings) {
                    if (err || !settings) { data.response = res.__('Jobs Not Available');
                        res.send(data); } else { callback(err, tasker, ratings, task, settings); }
                });
            },
            function(tasker, ratings, task, settings, callback) {
                db.GetOneDocument('currencies', { 'default': 1 }, {}, {}, function(err, currencies) {
                    if (err || !currencies) { data.response = res.__('Jobs Not Available');
                        res.send(data); } else { callback(err, tasker, ratings, task, settings, currencies); }
                });
            },
            function(tasker, ratings, task, settings, currencies, callback) {
                db.GetCount('review', { 'tasker': new mongoose.Types.ObjectId(request.provider_id), type: 'tasker', 'task': tasker._id }, function(err, count) {
                    if (err) { data.response = res.__('Jobs Not Available');
                        res.send(data); } else { callback(err, tasker, ratings, task, settings, currencies, count); }
                });
            },
            function(tasker, ratings, task, settings, currencies, count, callback) {
                db.GetCount('review', { 'tasker': new mongoose.Types.ObjectId(request.provider_id), type: 'tasker', 'task': task._id }, function(err, count) {
                    if (err) { data.response = res.__('Jobs Not Available');
                        res.send(data); } else { callback(err, tasker, ratings, task, settings, currencies, count, count); }
                });
            }
            ...
            data.response.job.photo = task.photo;
            data.response.job.url = settings.settings.site_url+'uploads/images/task/';
            res.send(data);
        });
    }
        ...
        async.waterfall([
            ...
            function(tasker, callback) {
                db.GetAggregation('review', [{ $match: { tasker: new mongoose.Types.ObjectId(request.provider_id) } },
                    { $group: { _id: '$tasker', average: { $avg: '$rating' }, total: { $sum: 1 } } }
            ...
            function(tasker, ratings, reviews, callback) {
                db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function(err, settings) {
                    if (err) { data.response = res.__('Invalid ' + CONFIG.TASKER + ', Please check your data');
                        res.send(data); } else { callback(err, tasker, ratings, reviews, settings); }
                });
            }
                ...
                async.parallel({
                    TotalJobs: function(callback) {
                        db.GetCount('task', { 'tasker': request.provider_id }, function(err, count) {
                            if (err || count == 0) { var count = 0;
                                callback(err, count); } else { callback(err, count); }
                        });
                    },
                    ...
                    if (err || !bookings) {
                        ...
                        db.GetOneDocument('task', { 'task_date': bookings.task_date, 'tasker': request.provider_id, 'task_hour': bookings.task_hour, 'status': { '$in': [2, 3, 4, 5] } }, {}, {}, function (err, existingbookings) {
                            if (!existingbookings) {
                                    ...
                                    async.parallel({
                                        ...
                                        UserReview: function (callback) {
                                            if (bookings.user._id) {
                                                db.GetAggregation('review', [
                                                    ...
                                                    if (err || UserReview[0]) { callback(err, UserReview[0]); } else { callback(err, UserReview[0]); }
                                                });
                                            } else {
                                                callback(null, '');
                                            }
                                        ...
                                        TaskerReview: function (callback) {
                                            db.GetAggregation('review', [
                                                ...
                                                if (err || TaskerReview[0]) { callback(err, TaskerReview[0]); } else { callback(err, TaskerReview[0]); }
                                            });
                                        }
                                    }, function (err, result) {
                                        if (err) {
                                ...
                                if (existingbookings.status == 2 || existingbookings.status == 3 || existingbookings.status == 4 || existingbookings.status == 5) {
                                    data.status = 0;
            ...
            if (err || !provider) {
                ...
                db.GetOneDocument('task', { 'booking_id': request.job_id }, {}, extension, function (err, bookings) {
                    if (err || !bookings) {
                        ...
                        // db.GetOneDocument('task', {  'tasker': request.provider_id,'status': { '$in': [2, 3, 4, 5] } }, {}, {}, function (err, existingbookings) {
                        //     if (existingbookings) {
                        //         data.status = 0;
                        //         data.response = res.__("You Cannot accept More than one task for a session");
                        //         res.send(data);
            ...
            if (err || !provider) {
                ...
                db.GetOneDocument('task', { 'booking_id': request.job_id }, {}, extension, function(err, bookings) {
                    if (err || !bookings) {
                        ...
                        db.GetDocument('task', { tasker: new mongoose.Types.ObjectId(request.provider_id), status:{ $gt :  2, $lt : 6} }, {}, {}, function (err, tasklastdata) {                          
                          if(tasklastdata.length >= 1){
                             data.response = res.__('Please complete the ongoing job');
                             res.send(data);
                            }
                            else{                           
                                                        ...
                                                        db.GetCount('review', { 'tasker': new mongoose.Types.ObjectId(request.provider_id), type: 'tasker', 'task': bookings._id }, function(err, count) {
                                                            ...
                                                            if (err) {
                                                                ...
                                                                if (bookings.tasker && (bookings.status == 6 || bookings.status == 3 || bookings.status == 7)) {
                                                                    ...
                                                                    data.response.job.photo = bookings.photo;
                                                                    data.response.job.url = settings.settings.site_url+'uploads/images/task/';
                        ...
                        var condition = [
                            ...
                            { $project: { 'task': '$task._id', 'booking_id': '$task.booking_id', 'Category': '$task.booking_information.work_type', 'message': 1, 'createdAt': 1, '_id': 1, 'tasktime': '$task.updatedAt' } },
                            {
                                $group: {
                                    ...
                                    'tasktime': { "$first": '$tasktime' },
                                    'messages': { $push: { "createdAt": "$createdAt", 'message': "$message" } }
                                }
                            },
                            { "$sort": { 'tasktime': -1 } },
                            { "$skip": (request.perPage * (request.page - 1)) },
