																					...
																					if (err || result.nModified == 0) {
																						...
																						if (balance_amount == 0) {
																							if (balance_amount == 0) {
																									...
																									var transaction = {
																										'user': data.user_id,
																										'tasker': taskdata[0].tasker,
																										'task': taskdata[0]._id,
																										'type': 'pay by cash',
																										'amount': balance_amount,
																										'task_date': taskdata[0].createdAt,
																										'status': 1
																									};
																										...
																										if (err || transaction.nModified == 0) { data.response = res.__('Error in data, Please check your data'); res.send(data); }
																											...
																											var paymenttype = "100% Coupon"
																											db.UpdateDocument('task', { 'booking_id': data.booking, 'user': data.user_id }, { $push: { transactions }, 'invoice.status': 1, 'status': 7, 'payment_type': paymenttype, 'invoice.amount.balance_amount': 0, 'history.job_closed_time': new Date() }, {}, function (err, upda) {
																												var provider_id = taskdata[0].tasker;
																												var message = CONFIG.NOTIFICATION.PAYMENT_COMPLETED;
																												var amount_to_receive = 0.00;
																												var currency = currencies.code;
																												var options = {
																													'job_id': req.body.booking_id,
																													'provider_id': provider_id,
																													'amount': amount_to_receive,
																													'currency': currency
																												};
																												console.log("=-=options====",options);
																												push.sendPushnotification(taskdata[0].tasker, message, 'payment_paid', 'ANDROID', options, 'PROVIDER', function (err, Response, body) { 
																												console.log("=-=Notification====",err, Response, body);
																												});
																											})
																											res.send({ "status": "2", "response": "Payment Completed Succesfully", "discount": (discount * currencies.value).toFixed(2) });
