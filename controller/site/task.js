...
module.exports = function (io) {
		...
		if (responseflag) {
			async.waterfall([
				...
				function (settingData, taskData, callback) {
					...
					defaultCondition = [
						{
							"$geoNear": {
								...
								query: {
									"status": { $in: [ 1, 2, 3 ] }, "availability": 1,
					...
					//console.log("taskercondition",taskercondition)
				...
				if (err) {
					...
					//console.log(({ count: taskerdata[0].count, result: newArray, avgrating: avgratingdata, taskercount: documentData, countall: newtaskercount }));
	...
	router.requestalltask = function requestalltask(req, res) {
		var content = req.body.data;
		db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settingdata) {
			if (err) {
				res.send(err);
			} else {
				content.booking_information.booking_date = new Date();
				content.booking_id = settingdata.settings.bookingIdPrefix + '-' + Math.floor(100000 + Math.random() * 900000);;
				var job_date = timezone.tz(content.booking_information.booking_date, settingdata.settings.time_zone).format(settingdata.settings.date_format);
				var job_time = timezone.tz(content.booking_information.booking_date, settingdata.settings.time_zone).format(settingdata.settings.time_format);
				var job_day = timezone.tz(content.booking_information.booking_date, settingdata.settings.time_zone).format('dddd');
				content.task_hour = job_time;
				content.task_date = job_date;
				content.task_day = job_day;
				console.log("Content", content);
				db.GetOneDocument('tasker', { _id: content.tasker }, {}, {}, function (err, taskerData) {
					if (err) {
						res.send(err);
					} else {
						db.InsertDocument('task', content, function (err, docdata) {
							if (err) {
								res.send(err);
							} else {
								//console.log("TaskerData", taskerData);
								var mailcredentials = {};
								mailcredentials.taskname = content.category.name + " (" + content.booking_information.work_type + ")";
								mailcredentials.username = content.user.username;
								mailcredentials.taskername = taskerData.username;
								mailcredentials.taskeremail = taskerData.email;
								mailcredentials.useremail = content.user.email;
								mailcredentials.bookingid = content.booking_id;
								mailcredentials.taskdate = job_date;
								mailcredentials.taskhour = job_time;
								mailcredentials.taskdescription = content.task_description;
								var username;
								var taskername;
								if (taskerData.name) {
									taskername = taskerData.name.first_name + " (" + taskerData.username + ")";
								} else { taskername = taskerData.username; }
								if (content.user.name) {
									username = content.user.name.first_name + " (" + content.user.username + ")";
								} else { username = content.user.username; }
								...
								var notifications = { 'job_id': content.booking_id, 'user_id': content.user._id };
								push.sendPushnotification(content.tasker, message, 'job_request', 'ANDROID', notifications, 'PROVIDER', function (err, response, body) { });
								res.send(docdata);
							}
						});
					}
				...
				if (err || !settingdata) {
					...
					db.GetDocument('task', { _id: req.body.data._id }, { booking_information: 1 }, {}, function (err, dateDate) {
						if (err) {
							...
							data.booking_information.booking_date = booking;
							//new code
																...
																if (err) {
																	...
																	db.GetOneDocument('category', { _id: taskdata[0].category.parent }, {}, options, function (err, category) {
																		if (err) {
																			...
																			var job_date = timezone.tz(booking, settingdata.settings.time_zone).format(settingdata.settings.date_format);
																			var job_time = timezone.tz(booking, settingdata.settings.time_zone).format(settingdata.settings.time_format);
																			var mailcredentials = {};
 ...
 router.uploadTaskImages = function uploadTaskImages(req, res) {
        if (req.file) {
            res.send({ "status": 1, "response": { file: req.file.filename } });
        } else {
            res.send({ "status": 0, "response": "Failed" });
        }
    }
