					...
					if (err) {
						...
						if(doc[0]){
							for(var i=0; i < doc[0].TaskDetails.length; i++){
							   if(doc[0].TaskDetails[i].photo.length){
								doc[0].TaskDetails[i].photo = doc[0].TaskDetails[i].photo.map(function(el) {
									return settings.settings.site_url+'uploads/images/task/' + el; 
								})
							   }
							}
							res.send(doc);
		...
		db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
			if (err || !settings) {
				...
				db.GetOneDocument('task', { _id: req.body.data.taskid }, {}, {}, function (err, task) {
					...
					db.GetDocument('task', { tasker: new mongoose.Types.ObjectId(task.tasker), status:{ $gt :  2, $lt : 6}, booking_id: {$ne:task.booking_id} }, {}, {}, function (err, tasklastdata) {                          
						if(tasklastdata.length >= 1){
							res.send({ 'error': 'Please complete the ongoing job' });
							}
							else{
																				...
																				if (err) {
																					...
																					for (var i = 0; i < data.length; i++) {
																						...
																						if (data[i].code == req.body.coupon) {
																							...
																							console.log("coupon", invoice.amount.discount)
																					...
																					console.log("service_tax settings", settingdata.settings.service_tax)
																					console.log("newtax", newtax)
																					//console.log("TaskData", taskdata[0].invoice)
					...
					if (err || !currencies) {
						...
						db.GetOneDocument('settings', { 'alias': 'general' }, {}, {}, function (err, settings) {
							...
							if (err || !settings) {
								...
								if (task.invoice.miscellaneous.length !== 0) {
									var getmiscellaneous = [];
									for (var j = 0; j < task.invoice.miscellaneous.length; j++) {

										getmiscellaneous.push({
											price:req.query.symbol + " "+(task.invoice.miscellaneous[j].price * req.query.value).toFixed(2),
											name:task.invoice.miscellaneous[j].name
									})
									}

									data.miscellaneous = getmiscellaneous;
								}else{
									data.miscellaneousemp = req.query.symbol + " " + 0;
								}
